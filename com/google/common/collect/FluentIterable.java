...
public abstract class FluentIterable<E> implements Iterable<E> {
   ...
   *
   * <p><b>{@code Stream} equivalent:</b> {@code stream.collect(MoreCollectors.onlyElement())}.
   *
   ...
   *
   * <p><b>{@code Stream} equivalent:</b> {@code stream.collect(MoreCollectors.toOptional())
   * .orElse(defaultValue)}.
   *
   ...
   * iterable may be smaller). For example, partitioning a fluent iterable containing {@code [a, b,
   * c, d, e]} with a partition size of 3 yields {@code [[a, b, c], [d, e]]} -- an outer fluent
   * iterable containing two inner fluent iterables of three and two elements, all in the original
   * order.
   *
   * <p>Iterators returned by the returned outer and inner fluent iterables do not support the
   ...
   * iterable containing {@code [a, b, c, d, e]} with a partition size of 3 yields {@code [[a, b,
   * c], [d, e, null]]} -- an outer fluent iterable containing two inner fluent iterables of three
   * elements each, all in the original order.
   *
   * <p>Iterators returned by the returned fluent iterables do not support the {@link
