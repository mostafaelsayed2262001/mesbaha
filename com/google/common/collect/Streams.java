...
public final class Streams {
  ...
  /**
   * Returns a <b>single-use</b> {@link Iterable}, which safely allows a stream to be used as the
   * target of a for-each loop: {@code for (Foo foo : iterating(fooStream))}. This is a shorthand
   * for {@code for (Foo foo : (Iterable<Foo>) fooStream::iterator)}, avoiding the need to write
   * {@code Foo} twice.
   *
   * <p>It is unsafe to use this method outside the context of a for-each loop header, as it
   * produces a faulty iterable that becomes unusable after a single call to {@link
   * Iterable#iterator}. (Also, this can already be done using {@code theStream::iterator} without
   * the need for the ugly cast shown above, anyway; you don't need this method.)
   *
   * <p>Even when used correctly, there is still often a better alternative to this method. After
   * doing what can be done via the stream's chaining methods, you should consider your options in
   * the following order:
   *
   * <ol>
   *   <li>Use the standard terminal operations such as {@link Stream#findFirst}, {@link
   *       Stream#noneMatch}, and {@link Stream#reduce(BinaryOperator)}.
   *   <li>Use {@link Stream#collect(Collector)} with a {@link Collector} that yields the desired
   *       result (for example, {@link MoreCollectors#onlyElement}). Only in rare cases should you
   *       need the three-argument form, or to implement your own {@link Collector}.
   *   <li>Use {@link Stream#forEach} or {@link Stream#forEachOrdered} if possible. This should work
   *       unless you need to throw a checked exception, access mutable local state, or
   *       short-circuit the loop.
   *   <li>Finally, fall back on writing a for-each loop using this method.
   * </ol>
   *
   * <p>Note that you should almost never need to invoke {@code stream.iterator()}.
   */
  @GoogleInternal
  // This is an intentional violation, with static analysis to ensure it's used in safe contexts.
  @SuppressWarnings("StreamToIterable")
  public static <T> Iterable<T> iterating(Stream<T> stream) {
